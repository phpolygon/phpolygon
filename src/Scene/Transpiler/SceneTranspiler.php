...
class SceneTranspiler
{
    ...
    public function toArray(Scene $scene): array
    {
        ...
        return [
            ...
            'entities' => array_values(array_map(
                ...
                // Skip declarations a prefab's build() emitted: the anchor carries
                // the prefab reference and regenerates them on load.
                array_filter(
                    $builder->getDeclarations(),
                    static fn(EntityDeclaration $decl): bool => !$decl->isGeneratedByPrefab(),
                ),
            )),
        ];
    }
    ...
    {
        // A prefab anchor serializes its authored INPUT (placement + variant /
        // gameplay components), NOT build()'s derived output — the geometry is
        // regenerated by the prefab's build() on load and never inlined here.
        $authored = $decl->getPrefabAuthored();
        $sourceComponents = $authored ?? $decl->getComponents();
