...
// Vulkan / SPIR-V variant of resources/shaders/source/fxaa.frag.glsl.
...
// Push-constant carries the inverse resolution of the input texture so we
// avoid creating an additional uniform buffer for one vec2.
layout(push_constant) uniform FxaaPushConstants {
    vec2 u_inverse_resolution; // (1/width, 1/height)
} pc;
...
{
    vec2 uv  = v_uv;
    vec2 inv = pc.u_inverse_resolution;
