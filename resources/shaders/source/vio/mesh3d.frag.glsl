...
vec3 computeSand(vec3 N, vec3 V, vec3 L, out float roughOut) {
    ...
    if (zone >= 2.75 && zone < 3.25) {
        ...
        base *= 0.45 + seam * 0.45 + dome * 0.18;     // mortar recesses go dark

        // Per-stone colour from the NEAREST cell id (aligned to the actual
        // stone), with a few stones markedly lighter/darker for natural variety.
        ...
        roughOut = COBBLE_ROUGH - dome * 0.05; // crowns slightly smoother (worn)
        return base;
    }
...
vec3 computeWater(vec3 N, vec3 V, vec3 L, out float alphaOut, out float roughOut) {
    ...
    // Reflection probe: the baked cube already holds sky (up) AND the city
    // skyline (horizon), so let it dominate the reflection — the water then
    // visibly mirrors the buildings, not just the sky tint.
    if (u_has_environment_map == 1) {
        vec3 envR = texture(u_environment_map, R).rgb;
        reflectColor = mix(reflectColor, envR, 0.8);
    }
